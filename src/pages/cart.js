						...
						<ul className="space-y-4">
							{cart?.map((item) => (
								<li className="flex items-center gap-4">
									<img src={`http://localhost:1337${item?.cover_image?.url}`} alt="" className="object-cover w-16 h-16 rounded"/>
									<div>
						...
						<div className="flex justify-end pt-8 mt-8 border-t border-gray-100">
							<div className="w-screen max-w-lg space-y-4">
								...
								<div className="flex justify-end" >
									<button onClick={() => router.push(`/checkout?amount=${totalAmount}`)} className="block px-5 py-3 text-sm text-gray-100 transition bg-gray-700 rounded hover:bg-gray-600">
  										Checkout
									</button>  
								</div>
							</div>
